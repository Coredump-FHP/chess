  ...
  describe '#valid_move?' do
   ...
   it 'should move diagonally rt-bottom to lt-top any number of steps on the board' do
    ...
    it 'should not stand still' do
     bishop = create_bishop(5,5)
      #(0..7).each do |dist|
      #  (0..7).each do |dist|
          # every spot on the board, will test not moving
      #    bishop = create_bishop(x,y)
      #    expect(bishop.valid_move?(dist, dist)).to (eq false)
      #    expect(bishop.valid_move?(dist, dist)).to (eq false), " expected (#{dist},#{dist}) for bishop not be able to stand still"
      #  end
      #end
    end
  end
